...
Set `SecurityLabelDisable=true` only on these three containers, each with an explicit comment in the quadlet explaining the specific constraint. All other containers (rag-proxy, rag-watcher, postgres) run with full SELinux enforcement.
...
- UBI-based containers (rag-proxy on UBI9, rag-watcher on UBI10, postgres on sclorg) do not need the workaround
