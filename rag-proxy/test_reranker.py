...
def test_rerank_preserves_schema():
    """Output schema matches input — source, chunk_id, chunk_total all present."""
    mod = _reload_reranker(RERANKER_ENABLED="true", RERANKER_TOP_N="5")
    results = _make_results(10)
...
def test_rerank_fewer_than_top_n():
    """When input has fewer results than top_n, return all of them."""
...
def test_disabled_no_reranker_score():
    mod = _reload_reranker(RERANKER_ENABLED="false")
...
def test_model_swap_via_env():
    ...
    ranked = mod.rerank("test query", results)
    assert len(ranked) == 3
    assert all("reranker_score" in r for r in ranked)
